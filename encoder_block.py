...
if __name__=='__main__':
    ...
    # print('emb_result:', emb_result.size()) #emb_result: torch.Size([1, 15, 128])

    attn_mask=torch.zeros((1,de_ids_tensor.size()[0],de_ids_tensor.size()[0])) # batch中每个样本对应1个注意力矩阵

    # 5个Encoder block堆叠
    encoder_blocks=[]
    for i in range(5): #下一个block的输入是上一个block的输出
    ...
    # print('encoder_outputs:',encoder_outputs.size()) #encoder_outputs: torch.Size([1, 15, 128])