...
class Decoder(nn.Module):
    ...
    def forward(self,x,encoder_z,encoder_x): # x: (batch_size,seq_len_q)
        """
        前向传播函数，用于计算模型输出。
        
        Args:
            x (torch.Tensor): 输入的目标序列，形状为 (batch_size, seq_len_q)。(译文答案)
            encoder_z (torch.Tensor): 编码器输出的隐藏状态。（原文的隐式表示）
            encoder_x (torch.Tensor): 编码器的输入序列。（原文）
        
        Returns:
            logits (torch.Tensor): 模型的输出。形状为 (batch_size, seq_len_q, vocab_size)。
        
        """
        first_attn_mask=(x==PAD_IDX).unsqueeze(1).expand(x.shape[0],x.shape[1],x.shape[1]).to(DEVICE) # 目标序列的pad掩码
        first_attn_mask=first_attn_mask|torch.triu(torch.ones(x.shape[1],x.shape[1]),diagonal=1).bool().unsqueeze(0).expand(x.shape[0],-1,-1).to(DEVICE) # &目标序列的向后看掩码
        # print("##first_attn_mask.shape:", first_attn_mask.shape)
        # 根据来源序列的pad掩码，遮盖decoder对其pad部分的注意力
        second_attn_mask=(encoder_x==PAD_IDX).unsqueeze(1).expand(encoder_x.shape[0],x.shape[1],encoder_x.shape[1]).to(DEVICE) # (batch_size,target_len,src_len) #(batch_size,译文_len,原文_len)
...
if __name__=='__main__':
    ...
    print('enc_x_batch batch:', enc_x_batch.shape)
    ...
    print('dec_x_batch batch:', dec_x_batch.shape)
    ...
    print('decoder outputs:', enc_outputs.shape)