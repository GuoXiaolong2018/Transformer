...
if __name__=='__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        for pad_enc_x,pad_dec_x in dataloader:
            # print("##pad_enc_x.shape:",pad_enc_x.shape) #torch.Size([250, 30])
            # print("##pad_dec_x.shape:", pad_dec_x.shape) # torch.Size([250, 37])

            # 解码器的输入是目标序列向右移动一个位置的版本。
            real_dec_z=pad_dec_x[:,1:].to(DEVICE) # decoder正确输出 #label ## torch.Size([250, 36])
            pad_enc_x=pad_enc_x.to(DEVICE) 

            #这是为了准备解码器的实际输入，即目标序列向右移动一个位置，模拟解码过程。
            pad_dec_x=pad_dec_x[:,:-1].to(DEVICE) # decoder实际输入  #torch.Size([250, 36])
            dec_z=transformer(pad_enc_x,pad_dec_x) # decoder实际输出
            # print("##dec_z.shape:",dec_z.shape) # torch.Size([250, 36, 10389])
